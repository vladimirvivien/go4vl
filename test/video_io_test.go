...
// TestIntegration_VideoInput consolidates all video input tests under one parent
// function that opens the device once, avoiding "device or resource busy" errors.
func TestIntegration_VideoInput(t *testing.T) {
	...
	t.Run("Info", func(t *testing.T) {
		...
		if err != nil {
			t.Skipf("GetVideoInputInfo not supported by driver: %v", err)
			return
		}
		...
		name := info.GetName()
		if name == "" {
			t.Log("Warning: Input name is empty (some drivers don't populate this)")
		}
...
// TestIntegration_VideoOutput consolidates all video output tests under one parent
// function that opens the device once.
func TestIntegration_VideoOutput(t *testing.T) {
	devPath := RequireV4L2Testing(t)
	dev := OpenDeviceOrSkip(t, devPath)
